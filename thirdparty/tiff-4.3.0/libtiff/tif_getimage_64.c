...
typedef void (*tileContigRoutine_64)
    (TIFFRGBAImage*, uint64_t*, uint32_t, uint32_t, uint32_t, uint32_t, int32_t,
     int32_t, unsigned char*);
typedef void (*tileSeparateRoutine_64)
    (TIFFRGBAImage*, uint64_t*, uint32_t, uint32_t, uint32_t, uint32_t, int32_t,
     int32_t, unsigned char*, unsigned char*, unsigned char*, unsigned char*);
 ...
 *
 * TODO: MERGE b7cc3e59027d960d6678a086638325f5629d8fa4 TO FIX CVE-2015-8665 AND CVE-2015-8683!
 */
static int
...
{
    return TIFFReadRGBAStripExt_64(tif, row, raster, 0 );
}

int
TIFFReadRGBAStripExt_64(TIFF* tif, uint32_t row, uint64_t * raster, int stop_on_error)

{
    ...
    if (TIFFRGBAImageOK(tif, emsg) && TIFFRGBAImageBegin_64(&img, tif, stop_on_error, emsg)) {
...
{
    return TIFFReadRGBATileExt_64(tif, col, row, raster, 0 );
}


int
TIFFReadRGBATileExt_64(TIFF* tif, uint32_t col, uint32_t row, uint64_t * raster, int stop_on_error )
{
    char 	emsg[1024] = "";
    TIFFRGBAImage img;
    int 	ok;
    ...
    if (!TIFFRGBAImageOK(tif, emsg) 
	|| !TIFFRGBAImageBegin_64(&img, tif, stop_on_error, emsg)) {
	    TIFFErrorExt(tif->tif_clientdata, TIFFFileName(tif), "%s", emsg);
	    return( 0 );
    }
